...
def expand_tile(tile)
  tile == BOX ? [BOX_L, BOX_R] : ([tile] * 2)
end

def expand_warehouse(warehouse)
  {
    init_loc: warehouse[:init_loc] + warehouse[:init_loc].real,
    mvts: warehouse[:mvts],
    rows: warehouse[:rows].map do |row|
      row.chars.each_with_object([]) { |t, ts| ts.concat(expand_tile(t)) }.join
    end
  }
end
...
def calc_row_gps_sum(row, idx)
  row.chars.map.with_index { |t, x| t == BOX_L ? 100 * idx + x : 0 }.sum
end

#puts #run_warehouse(
puts  parse_input($stdin.read)
